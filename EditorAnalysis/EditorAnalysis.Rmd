...
#TODO: Code publishers by exclusive OA PUBLISHER...
...
```{r code-roles}
    ...
    role.tbl %<>% rowwise() %>%
      ...
      str_detect(rolec,"Editorial Office") ~ "",
...
gender_meth_list <- c("ipums","ssa","kantrowitz","genderizer")
gender_meth <- "ipums"
#TODO: 
...
safer_gender <- function(x,...) {
  rv <- c(gender=NA,proportion_male=NA)
  ...
  rsg <- safe_gender(x,...)
  if (c("gender"names(rsh)) {
    rv <- c(gender=NA,proportion_male=NA)
  } else if (is.na(rv$gender)) {
    rv <- c(gender=NA,proportion_male=NA)
  } else {}
  
  if (!"proportion_male" %in% colnames())
  rv %<>% select(gender,proportion_male)
  rv
}
if (doc_refresh_data){
...
person_ids.tbl <- editors_analysis.tbl %>% 
  ...
  #group_by(LS_FULLNAME,LS_INSTITUTION) %>%
    group_by(LS_FULLNAME,affiliation) %>%
  count() %>% ungroup() %>% 
  filter(n>1) %>% 
  mutate(editor_id = seq_len(n())) %>%
#  select(-n) %>%
...
editors_analysis.tbl %<>% mutate (
  ...
  select(NM_JOURNAL, CAT_PUBLISHER,FAC_TOP, IND_MALE, IND_OPEN, LS_COUNTRY, LS_SUBJECTS, FAC_ROLE, FAC_TOP,NM_EDID) %>% ungroup()
```
