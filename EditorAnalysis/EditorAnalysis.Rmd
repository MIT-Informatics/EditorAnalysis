...
gender_meth_list <- c("ipums","ssa","kantrowitz","genderize")


# gender can fail on genderize method, return inconsistent number of columns depending on method
...
gender_meth <- "ssa"
editors_full.tbl %<>% left_join(gender_select.bl <- nmsgender.tbl %>% filter(method==gender_meth) %>% select(name,gender,proportion_male) %>% rename(LS_GIVENNAME=name,LS_GENDER=gender,LS_PROPENSITIY_MALE=proportion_male)
, by=c("LS_GIVENNAME"))
...
person_ids.tbl <- person_ids_full.tbl %>% filter(n>1) #%>% select(-n)
...
editors_analysis.tbl %<>% mutate (
  ...
  select(NM_JOURNAL, CAT_PUBLISHER,FAC_TOP, IND_MALE, IND_OPEN, LS_COUNTRY, LS_SUBJECTS, FAC_ROLE, FAC_TOP,NM_EDID,LS_PROPENSITIY_MALE) %>% ungroup()
```
...
```
```{r network-gen}
#TODO: fuzzy network edge detection
...
journal_analysis.tbl %>% Desc(~CAT_PUBLISHER,data=.)
```
...
journal_analysis.tbl %>%
  ...
  summarize(n_journal=n(),
         ...
         mn_boards_all = mean(N_BRD_SIZE,na.rm=TRUE),
         range_boards_all =paste((range(N_BRD_SIZE, na.rm=TRUE)),collapse="-")
  ) %>% gt() %>% 
...
```{r}
#editors_analysis.tbl %>% Desc(FAC_ROLE~IND_MALE,data=.)
#editors_analysis.tbl %>% gf_percents(~IND_MALE|FAC_ROLE,na.rm)
editors_analysis.tbl %>% gf_percents(~FAC_ROLE,fill=~IND_MALE)
...
boardDiverse.tbl <-  journal_board_analysis.tbl %>%
  ...
  summarize(n_journal=n(),
         ...
         international =  mean(PERCENT_HHI,na.rm=TRUE),
       
  ) %>%
  pivot_longer(cols=c(male,international),names_to="divtype",values_to="per") %>%
...
boardDiverse.tbl %>%
    group_by(LABEL_SUBJECT) %>% mutate(subj_order=min(per,na.rm=TRUE)) %>%
...
editors_analysis.tbl %>% 
  ...
  summarize(pmale = mean(IND_MALE,na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(values_from=pmale, names_from=IND_OPEN) %>%
  group_by(IND_TOP) %>%
  gt() %>% 
    ...
    tab_header(title="Percent Male by OA and TOPS status") %>%
    data_color (
...
clustjournals.tib %>% unnest(LIST_EDID) %>% count(LIST_EDID,sort=TRUE) %>%
   count(n) %>% 
   gt %>% 
    cols_label(n="Number of boards",nn="") %>%
    tab_header(title="Journal Subject of Large Clusters") %>%
    gt_theme_538()
