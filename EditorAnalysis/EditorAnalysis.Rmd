...
era.tbl %<>% unite("Subjects",starts_with("FoR"),sep=",",na.rm=TRUE) %>%
  pivot_longer(starts_with("ISSN"), names_to = NULL, values_to = "ISSN", values_drop_na = TRUE )
...
editors.tbl %<>% rowwise() %>% mutate(NM_EDITOR=editor)
...
# - check for  name clustering that might indicate shared editorial boards across journals
# - compute gender only on unique names, join back in 

editors.tbl %<>% rowwise() %>% mutate(LS_NM_EDITOR=`editor`)
editors.tbl %<>% rowwise() %>% mutate(LS_NM_EDITOR_FIRST=safe_first_name(`LS_NM_EDITOR`))

#TODO

nms.tbl <-  editors.tbl %>% count(LS_NM_EDITOR_FIRST) %>% arrange(desc(n))
...
nms.tbl %>% rowwise() %>%
  mutate( LS_NM_EDITOR_GENDER = 
            safe_gender(`LS_NM_AUTHOR_FIRST`,method=gender_meth)[["gender"]])
