...
journal_analysis.tbl <- journal_board_analysis.tbl %>%
  ...
  slice_head()  %>%
  rowwise() %>%
  mutate(LIST_EDID2=ifelse(length(unlist(LIST_EDID))==0, NA,  list(LIST_EDID)))%>%
  ungroup() %>% 
...
library("igraph")
library("ggraph")
edges.df <- journal_analysis.tbl %>% select(LIST_EDID,NM_JOURNALID) %>% unnest(LIST_EDID)
...
gr.values <-create_graph(
  nodes_df = df.nodes.values , 
  edges_df = df.edges.values, 
  graph_name="Journal Board Sharing",
  directed=FALSE)

tmp1<- gr.values %>% render_graph()
```
