  ...
  boardDiverse.tbl %>%
    ...
    facet_wrap(facets=vars(divtype)) +
     labs(x = "Diversity", y="",
       ...
       color = "Role", caption="Note: Chief editors are pooled across discipline.") 
...
p1<- tmp %>% gf_dens2(~International|FAC_ROLE,fill=~OA)
p2<- tmp %>% gf_dens2(~Female|FAC_ROLE,fill=~OA)
p3<- tmp %>% gf_dens2(~International|FAC_ROLE,fill=~TOP)
p4<- tmp %>% gf_dens2(~Female|FAC_ROLE,fill=~TOP)
plot((p1+p2)/(p3+p4))
