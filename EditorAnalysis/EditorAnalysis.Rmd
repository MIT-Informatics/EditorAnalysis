...
```{r code-roles}
    ...
    role.tbl %<>% rowwise() %>%
      ...
      str_detect(rolec,"Book Review") ~ "editor",F
...
journal_board_analysis.tbl <- editors_analysis.tbl %>% 
  ...
  LIST_ROLEGROUP_PROPENSITY_MALE = list(PERCENT_PROPENSITY_MALE),
...
tb1.gt 
tb1.gt %>% gtsave(filename="tb1.htm")
tb1.gt %>% gtsave(filename="tb1.pdf")
tb1.gt %>% gtsave(filename="tb1.png")
tb1.gt %>% gtsave(filename="tb1.rtf")

journal_analysis.tbl %>% filter(CAT_PUBLISHER=="PLOS")

editors_analysis.tbl %>% filter(CAT_PUBLISHER=="PLOS") %>% count(NM_JOURNAL)
...
#TODO: Fix crosstable kludge -- cleaner with group_by?
 ...
 tb2.gt
 tb2.gt %>% gtsave(file="tb2.htm",inline_css=TRUE)
  tb2.gt %>% gtsave(file="tb2_1.htm",inline_css=FALSE)

  
 
 # percentage of 20 countries
 as.tibble(edcountry.ct) %>% select(variable,Total) %>% mutate(Total=as.numeric(Total)) %>% slice_max(Total,n=20) %>% summarize(sum(Total)) %>% pull / as.tibble(edcountry.ct) %>% select(variable,Total) %>% mutate(Total=as.numeric(Total)) %>% summarize(sum(Total)) %>% pull
```
...
editors_analysis.tbl %>% slice_sample(n=1000) %>% gf_percents(~FAC_ROLE,fill=~LS_COUNTRY)
   
# identified gender
 (editors_analysis.tbl %>% filter(!is.na(IND_MALE)) %>% count ) / nrow(editors_analysis.tbl)
 editors_analysis.tbl %>% filter(!is.na(IND_MALE)) %>% filter(!is.na(FAC_ROLE))  %>% crosstable(cols=c(IND_MALE)) %>% as_gt()
 
  editors_analysis.tbl %>% filter(!is.na(IND_MALE)) %>% filter(!is.na(FAC_ROLE))  %>% crosstable(cols=c(IND_MALE),by=FAC_ROLE, margin="column",total="column") %>% as_gt()

  # all roles
  hhi(as.tibble(edcountry.ct) %>% select(variable,Total) %>% mutate(share=100*as.numeric(Total)/sum(as.numeric(Total))) %>% as.data.frame(),"share")

  # editor in chief
  hhi(as.tibble(edcountry.ct) %>% select(variable,chief) %>% mutate(share=100*as.numeric(chief)/sum(as.numeric(chief))) %>% as.data.frame(),"share")/10000
...
journals_linked.tgraph %>%
  activate(nodes) %>%
  mutate(Publisher=factor(CAT_PUBLISHER)) %>%
  ggraph(layout="stress") +
    geom_edge_fan(alpha=.2) +
    geom_node_point(mapping=aes(color=Publisher)) +
                      scale_color_viridis_d() + theme(legend.position ="bottom") -> fig6

fig6
fig6 %>% ggsave(plot=., filename="figure_6.png" , units="in", width=7, height=7,scale=2)
...
boardDiverse.tbl <-  journal_board_analysis.tbl %>%
  ...
  summarize(
         female = 1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE),
         international =  1-mean(PERCENT_HHI,na.rm=TRUE),
       
  ) %>% ungroup()


# pool chief editors across field for diversity calculation
boardChief_1.tbl <- journal_board_analysis.tbl %>%
  filter(FAC_ROLE=="chief") %>% 
  unnest(LIST_SUBJECTS) %>% 
  unnest(LIST_ROLEGROUP_PROPENSITY_MALE) %>% 
  group_by(LABEL_SUBJECT,FAC_ROLE) %>% 
  summarize(female=1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE)) %>%
  ungroup()

boardChief_2.tbl <- journal_board_analysis.tbl %>%
  filter(FAC_ROLE=="chief") %>% 
  unnest(LIST_SUBJECTS) %>% 
  unnest(LIST_ROLEGROUP_COUNTRIES) %>% 
  group_by(LABEL_SUBJECT,FAC_ROLE) %>% 
  summarize(international =1-countries_to_hhi(COUNTRIES)) %>%
  ungroup()

boardDiverse.tbl %<>% bind_rows(left_join(boardChief_1.tbl,boardChief_2.tbl)) %>%
 pivot_longer(cols=c(female,international),names_to="divtype",values_to="per") %>%
  ...
  arrange(LABEL_SUBJECT,FAC_ROLE,divtype)
...
boardDiverse.tbl %>%
  ...
  gt() %>% 
    cols_label( "LABEL_SUBJECT"="",
                ...
                "review_international"="Review board: % national diversity",
                "editor_female"="Editorial board: % female" , 
                "editor_international"="Editorial board: % national diversity",
                "chief_female"="Chief editors: % female" , 
                "chief_international"="Chief editors: % national diversity",
                ) %>%
  ...
  boardDiverse.tbl %>%
    ...
    facet_wrap(facets=vars(divtype)) +
     labs(x = "Diversity", y="",
       ...
       color = "Role", caption="Note: Chief editors are pooled across discipline.") -> fig3
       fig3
       
       fig3 %>% ggsave(plot=., filename="figure_3.png" , units="in", width=7,height=3.5,scale=2)
...
countries_to_hhi_ci_rs <- function(x, rep=1000) {
  if (!inherits(x,"data.frame")) {
    x <- tibble(COUNTRIES=x)
  }
  est <- countries_to_hhi(x)

  hhi_split <- function(split, ...) {
    res <- split %>% analysis() %>% countries_to_hhi()
   tibble(
    term = "hhi",
    estimate = res,
    std.error = NA_real_ # unknown
  )
 }
  
  y<- x %>%
    bootstraps( times = rep, apparent = TRUE) %>%
    mutate(models = map(splits, function(x) hhi_split(x)))
  res <- int_bca(y,models,.fn=hhi_split)
  tibble(y=est,ymin=res$.lower, ymax=res$.upper)
}

journal_board_analysis_m.tbl <-
  journal_board_analysis.tbl %>% 
  ungroup() %>%
  filter(!is.na(FAC_ROLE)) %>% 
  mutate(IND_TOP=(FAC_TOP>"no TOP")) %>%
  mutate(IND_TOP = factor(IND_TOP)) %>%
  mutate(IND_OPEN = factor(IND_OPEN)) %>%
  mutate(FEMALE = 1-PROPENSITY_ROLEGROUP_MALE, INTERNATIONAL = 1-PERCENT_HHI) %>%
  select(FAC_ROLE,IND_TOP,IND_OPEN,FEMALE,INTERNATIONAL,LIST_ROLEGROUP_PROPENSITY_MALE, LIST_ROLEGROUP_COUNTRIES)

# pool chief editors across field for diversity calculation
boardChief_f.tbl <- journal_board_analysis_m.tbl %>%
  filter(FAC_ROLE=="chief") %>% 
  mutate(FAC_ROLE="chief-pooled") %>%
  unnest(LIST_ROLEGROUP_PROPENSITY_MALE) %>%
  mutate(FEMALE=1-LIST_ROLEGROUP_PROPENSITY_MALE) %>%
  group_by(FAC_ROLE, IND_OPEN,IND_TOP) %>%
  summarize(res= mean_cl_boot(FEMALE,na.rm=TRUE), group_n=n()) %>%
  unpack(res) %>%
  ungroup()

boardChief_i.tbl <- journal_board_analysis_m.tbl %>%
  filter(FAC_ROLE=="chief") %>% 
  mutate(FAC_ROLE="chief-pooled") %>%
  unnest(LIST_ROLEGROUP_COUNTRIES) %>%
  group_by(FAC_ROLE, IND_OPEN,IND_TOP) %>% 
  summarize(res = countries_to_hhi_ci_rs(COUNTRIES), group_n=n()) %>%
  unpack("res") %>%
   mutate(y=1-y,oy=ymax, ymax=1-ymin,ymin=1-oy) %>% select(-oy) %>%
   ungroup()

journal_board_analysis_m.tbl %>% 
  filter(!is.na(FAC_ROLE)) %>%
  group_by(FAC_ROLE,IND_OPEN,IND_TOP) %>%
  summarize( res = mean_cl_boot(INTERNATIONAL,na.rm=TRUE),group_n=n()) %>% 
  unpack(res) %>%
  ungroup() %>%
  bind_rows(boardChief_i.tbl) %>%
  mutate(IND_TOP=fct_recode(IND_TOP, 
        "TOP Open Science"="TRUE", "Not Open Science"="FALSE"),
         measure = "MULTINATIONAL") -> board_i.tbl
...
journal_board_analysis_m.tbl %>% 
  ...
  unpack(res) %>%
  ungroup() %>%
  bind_rows(boardChief_f.tbl) %>%
  mutate(IND_TOP=fct_recode(IND_TOP, 
        "TOP Open Science"="TRUE", "Not Open Science"="FALSE"),
         measure = "FEMALE") -> board_f.tbl

 board_i.tbl %>%
  pivot_wider(values_from=c("y","ymin","ymax","group_n"), names_from=IND_OPEN)%>%
  group_by(IND_TOP) %>%
  gt() %>% 
    ...
    cols_hide(measure) %>%
    tab_spanner("Open Access",columns = ends_with("_TRUE")) %>%
    tab_spanner("Closed Access",columns = ends_with("_FALSE")) %>%
    cols_merge_range("ymin_TRUE","ymax_TRUE") %>%
    cols_label(ymin_TRUE="(95% CI)") %>%
      cols_label(ymin_FALSE="(95% CI)") %>%
    cols_label(group_n_TRUE="(n)") %>%
      cols_label(group_n_FALSE="(n)") %>%
    cols_merge_range("ymin_FALSE","ymax_FALSE") %>%
    fmt_percent(columns=matches("mean|ymin|ymax"),decimals=0) %>%
    tab_header(title="Board International Diversity by OA and TOPS status") %>%
    tab_source_note("Note: Diversity measures are computed per-board for editorial and review boards, and pooled for chief-editor roles.") %>%
    data_color (
    ...
    gt_theme_538() -> tba1.gt
 
 tba1.gt
   tba1.gt %>% gtsave("tba1.htm",inline_css=FALSE)
 ...
 board_f.tbl %>%
  ...
  gt() %>% 
    ...
    cols_merge_range("ymin_FALSE","ymax_FALSE") %>%
    fmt_percent(columns=matches("mean|ymin|ymax"),decimals=0) %>%
    tab_header(title="Board Gender Diversity by OA and TOPS status") %>%
    tab_source_note("Note: Diversity measures are computed per-board for editorial and review boards, and pooled for chief-editor roles.") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
    ...
    gt_theme_538() -> tba2.gt
 
 tba2.gt

 tba2.gt %>% gtsave("tba2.htm",inline_css=FALSE)


```

```{r board-comparisons-fig}
boardn <- dim(journal_board_analysis_m.tbl)[[1]]
bind_rows(board_f.tbl,board_i.tbl) %>%
  mutate(  IND_OPEN=fct_recode(IND_OPEN, 
        "Open Access"="TRUE", "Closed Access"="FALSE")) %>%
  filter(FAC_ROLE!="chief") %>%
  mutate(FAC_ROLE=fct_recode(FAC_ROLE,"chief"="chief-pooled")) %>%
 mutate(IND_TOP= fct_recode(IND_TOP, 
        "Open Science Policy"="TOP Open Science", "No Policy"="Not Open Science"))%>% 
  mutate(IND_TOP= fct_relevel(IND_TOP,"Open Science Policy"))%>% 
  ggplot(aes(x=fct_relevel(FAC_ROLE,"chief","editor","review"),
             y=y, colour=IND_OPEN)) + 
    geom_errorbar(aes(ymin=ymin, ymax=ymax)) +
    geom_point(aes(size=group_n)) +
    #geom_label(aes(label=round(y,digits = 2)), size=2) +
    facet_grid(cols=vars(IND_TOP), rows=vars(measure), scales="free_y") +
    labs(y="Diversity",x="Journal Board Type",title="Diversity of Journal Boards",
         caption = str_wrap(paste0("Points show the mean diversity of individual boards within category for review and editorial boards; and diversity across pooled category for chief-editors. Bars represent 95% CI. ", "N = ", boardn),100))+
  theme(legend.title = element_blank()) -> fig5

fig5 %>% ggsave(plot=., filename="figure_5.png" , units="in", width=7,height=3.5,scale=2)
fig5 %>% ggsave(plot=., filename="figure_5.eps" , units="in", width=7,height=3.5,scale=2)
fig5 %>% ggsave(plot=., filename="figure_5.svg" , units="in", width=7,height=3.5,scale=2)
...
board_f.tbl %>% filter(FAC_ROLE !="chief") %>% summarize(grand_mean_f=mean(y)) %>% as.vector
board_i.tbl %>% filter(FAC_ROLE !="chief") %>% summarize(grand_mean_i=mean(y)) %>% as.vector


board_f.tbl %>% filter(FAC_ROLE !="chief", FAC_ROLE!="chief-pooled") %>% summarize(grand_mean_f_nc=mean(y)) %>% as.vector
board_i.tbl %>% filter(FAC_ROLE !="chief", FAC_ROLE!="chief-pooled") %>% summarize(grand_mean_i_nc=mean(y)) %>% as.vector

journal_board_analysis_m.tbl %>% 
  filter(!is.na(FAC_ROLE)) %>%
  group_by(FAC_ROLE) %>%
  summarize( meanf = mean(FEMALE,na.rm=TRUE))

journal_board_analysis_m.tbl %>% 
  filter(!is.na(FAC_ROLE)) %>%
  group_by(FAC_ROLE) %>%
  summarize( meani = mean(INTERNATIONAL,na.rm=TRUE))

```


```{r board-comparisonaov-appendix}
library(broom)
board_cmp_aovs.tbl <- pmap_dfr (
  expand_grid( role=levels(journal_board_analysis_m.tbl$FAC_ROLE),
               dep=c("FEMALE","INTERNATIONAL")) ,
  function(role,dep) {
     journal_board_analysis_m.tbl %>%
           filter(FAC_ROLE == {{role}} ) %>%
           aov(data=., FEMALE  ~  IND_TOP + IND_OPEN ) %>% 
           tidy %>% mutate(ROLE=role, DEP = {{dep}} ) %>% relocate(DEP,ROLE)
  }
)

board_cmp_aovs.tbl %>% filter(term!="Residuals") %>% select(-df,-sumsq,-meansq) %>%
  mutate(term=factor(term,levels=c("IND_TOP","IND_OPEN"), labels=c("TOP Open Science","Open Access"))) %>%
  group_by(ROLE, DEP) %>% gt %>%
  cols_label( "p.value" = "P-Value",
   ...
   fmt_number("statistic", decimals=2) %>%
     fmt("p.value",
          fns = function(x) { paste0("p=", signif(x,3)) }
         ) %>%
    tab_header(title="TOP Open Science and Open Access as a predictor of diversity") %>%
    tab_source_note("Note: Separate ANOVA's are run for each editorial board and index of diversity.") %>%
    gt_theme_538()-> aovtb.gt

 aovtb.gt
 aovtb.gt %>% gtsave("tba1.htm",inline_css=FALSE)
...
journal_diag.tbl <- journal_board_analysis.tbl %>% 
  filter(!is.na(FAC_ROLE),!is.na(FAC_TOP)) %>%
  filter(FAC_ROLE!="chief") %>%
   unnest(LIST_SUBJECTS) %>% 
  mutate(IND_TOP=(FAC_TOP>"no TOP")) %>%
  mutate(IND_TOP=ifelse(IND_TOP,"TOP Open Science","Not Open Science")) %>%
  group_by(IND_OPEN,FAC_ROLE,IND_TOP,LABEL_SUBJECT) %>%
  summarize( ihhi = 1-mean(PERCENT_HHI,na.rm=TRUE),prop_female = 1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(values_from=c(ihhi,prop_female), names_from=IND_OPEN) 

subjectweights.tbl <- journal_board_analysis.tbl %>% 
  filter(!is.na(FAC_ROLE),!is.na(FAC_TOP)) %>%
  unnest(LIST_SUBJECTS) %>% 
  mutate(IND_TOP=(FAC_TOP>"no TOP")) %>%
  mutate(IND_TOP=ifelse(IND_TOP,"TOP Open Science","Not Open Science")) %>%
  group_by(IND_OPEN,IND_TOP,LABEL_SUBJECT) %>%
  summarize( n_entries = n()) %>%
  group_by(IND_TOP,IND_OPEN) %>% 
  mutate(tot_entries=sum(n_entries,na.rm=TRUE)) %>%
  ungroup() %>% 
  mutate(prop_entries=n_entries/tot_entries) %>%
  select(-tot_entries,-n_entries) %>%
  pivot_wider(values_from=c(prop_entries), names_from=IND_OPEN, values_fill=0,names_prefix="weight_")  
  
left_join(journal_diag.tbl,subjectweights.tbl) %>%
  group_by(IND_TOP) %>%
  gt() %>% 
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Board International Diversity by Field OA and TOPS status") %>%
    cols_label( "prop_female_TRUE" = "OPEN",
                "prop_female_FALSE" = "CLOSED",
                  "ihhi_TRUE"="OPEN",
                  "ihhi_FALSE"="CLOSED") %>% 
    tab_spanner(columns=matches("prop_female"), label="Proportion Female") %>%
       tab_spanner(columns=matches("ihhi_"), label="Country Diversity") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
...
journal_diag.tbl %>% 
  rowwise() %>% 
  mutate(country_open_diff = (ihhi_TRUE>ihhi_FALSE),
         female_open_diff = prop_female_TRUE>prop_female_FALSE) %>%
  ungroup() %>%
  summarise(country_open_pos_prop=mean(country_open_diff, na.rm=TRUE),
            female_open_pos_prop=mean(female_open_diff, na.rm=TRUE))


left_join(journal_diag.tbl,subjectweights.tbl) %>%
  group_by(IND_TOP,FAC_ROLE) %>% 
  summarise(ihhi_CLOSED= mean(ihhi_FALSE,na.rm=TRUE),
            ihhi_OPEN=mean(ihhi_TRUE,na.rm=TRUE),
            ihhi_OPEN_WT=weighted.mean(ihhi_TRUE,weight_FALSE,na.rm=TRUE)) %>%
  gt()

left_join(journal_diag.tbl,subjectweights.tbl) %>%
  group_by(IND_TOP,FAC_ROLE) %>% 
  summarise(fem_CLOSED=mean(prop_female_FALSE,na.rm=TRUE),
            fem_OPEN=mean(prop_female_TRUE,na.rm=TRUE),
            fem_OPEN_WT=weighted.mean(prop_female_TRUE,weight_FALSE,na.rm=TRUE)) %>%
  gt()


```
```{r chief-editor-pool}
# Chief Editor Pool Analysis
chief_pool.tbl <- journal_board_analysis.tbl %>% 
  filter(!is.na(FAC_ROLE),!is.na(FAC_TOP)) %>%
  filter(FAC_ROLE=="chief") %>%
  mutate(IND_TOP=(FAC_TOP>"no TOP")) %>%
  mutate(IND_TOP=ifelse(IND_TOP,"TOP Open Science","Not Open Science")) 
 ...
 left_join (
  chief_pool.tbl %>% 
  unnest(LIST_ROLEGROUP_PROPENSITY_MALE) %>%
  group_by(IND_OPEN,IND_TOP) %>%
  summarize(prop_female = 1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE)) %>%
  ungroup(),
  chief_pool.tbl %>% 
  unnest(LIST_ROLEGROUP_COUNTRIES) %>%
  group_by(IND_OPEN,IND_TOP) %>%
  summarize(ihhi = 1-countries_to_hhi(COUNTRIES))
 ) %>%
  pivot_wider(values_from=c(prop_female,ihhi), names_from=IND_OPEN,  
              ) %>%
  group_by(IND_TOP) %>%
  gt() %>% 
    fmt_percent(columns=matches("FALSE|TRUE"), decimals=0) %>%
    ...
    tab_header(title="Chief Editor Pool Diversity by OA and TOPS status") %>%
    tab_spanner(columns=matches("prop_female"), label="Proportion Female") %>%
       tab_spanner(columns=matches("ihhi_"), label="Country Diversity") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
  ...
  gt_theme_538() -> tb3c.gt
 tb3c.gt
 tb3c.gt %>% gtsave("tb3c.htm",inline_css=FALSE)
...
tmp <- journal_board_analysis.tbl %>% 
  filter(!is.na(FAC_ROLE), FAC_ROLE!="chief") %>%
  mutate(IND_TOP=(FAC_TOP>"no TOP")) %>%
  mutate(International=1-PERCENT_HHI,
         Female=1-PROPENSITY_ROLEGROUP_MALE,
         OA=IND_OPEN,
         TOP=IND_TOP)
...
p1<- tmp %>% gf_dens2(~International|FAC_ROLE,fill=~OA) + theme(legend.position="") + labs(x="")
p2<- tmp %>% gf_dens2(~Female|FAC_ROLE,fill=~OA)+ labs(y="") + labs(x="")
p3<- tmp %>% gf_dens2(~International|FAC_ROLE,fill=~TOP)+ theme(legend.position="") 
p4<- tmp %>% gf_dens2(~Female|FAC_ROLE,fill=~TOP)+ labs(y="")
(p1+p2)/(p3+p4) -> fig4

fig4

fig4 %>% ggsave(plot=., filename="figure_4.png" , units="in", width=7,height=3.5,scale=2)


#TODO: Compare HHI index aggregate
```
...
jcomp.sizes %>% unlist %>%  as.tibble %>% group_by(value) %>%
  ...
  gt %>% 
    ...
    gt_theme_538()  -> tb5.gt
tb5.gt
tb5.gt %>% gtsave("tb5.htm",inline_css=FALSE)
...
clustjournals_all.tib %>% unnest(LIST_EDID) %>% count(LIST_EDID,sort=TRUE) %>%
 ...
 tb4.gt %>% gtsave(file="tb4.htm",inline_css=TRUE)
  tb4.gt %>% gtsave(file="tb4_1.htm",inline_css=TRUE)
...
# % of journals in clusters between 2-3
jcomp.sizes %>% unlist %>%  as.tibble %>% group_by(value) %>% summarize(v=sum(value)) %>% slice_head(n=2) %>% summarize(sum(v)) / dim(journal_analysis.tbl)[1]
```  
