...
# remove honorifics
editors_parse.tbl %<>% rowwise() %>% mutate( LS_FULLNAME = str_squish(str_replace( `editor`, '(Dr\\.)|(Prof\\.)|(Doctor)|(Professor)|(Dr )|(Prof )', '')))                                    editors_parse.tbl %<>% rowwise() %>% mutate(LS_GIVENNAME = safe_first_name(`LS_FULLNAME`))
                                             
# for diagnostics
names.tbl <- editors_parse.tbl %>% group_by(`LS_GIVENNAME`) %>% count()
...
editors_parse.tbl %<>% rowwise() %>% mutate(LS_GIVENNAME = case_when(
  ...
  str_detect(LS_GIVENNAME,'.*(\\.|\\,)') ~ NA_character,
  TRUE ~ LS_GIVENNAME
))
```
...
affiliations.tbl %<>% rowwise() %>% mutate(LS_COUNTRY_CHK2 = str_to_title(LS_COUNTRY_CHK),
  ...
  str_detect(LS_COUNTRY_CHK2,"Trinidad") ~ "UY",
...
editor_full.tbl <- editors_parse_c.tbl
if (!doc_debug) {
...
#TODO:
# - compute gender only on unique names, join back in 

#TODO

nms.tbl <-  editors.tbl %>% count(`LS_GIVENNAME`) %>% arrange(desc(n))

nms.tbl %>% rename( n_names = n) %>%  group_by(n_names) %>% 
  summarize(cluster_size=n(),
            cluster_names=paste(LS_NM_EDITOR_FIRST,sep=",",collapse=","))  %>% arrange(desc(n_names)) 

nms.tbl %>% rowwise() %>%
  mutate( LS_GENDER = 
            safe_gender(`LS_GIVENNAME`,method=gender_meth)[["gender"]])
```
...
```
Distribution of editor names
```{r}
 editors.tbl %>% count(LS_NM_EDITOR_FIRST) %>% arrange(desc(n))
```
Affiliation matches
```{r}
tmp<-affiliations.tbl %>% filter(is.na(LS_COUNTRY_G)) %>% group_by(LS_COUNTRY_CHK)%>% count() %>% arrange(desc(n))
```
