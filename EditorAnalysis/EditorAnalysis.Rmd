...
tops_signatory_clean.tbl <- tops_signatory_raw.tbl %>%
  rename(journal="Journal Title", publisher="Publisher") %>%
...
#debug(issnClean)
tops_factor_clean.tbl %>% filter(is.na(issn)) %>% count() %>% pull
tops_factor.tbl <- issnClean(tops_factor_clean.tbl)
tops_factor.tbl %>% filter(is.na(issn)) %>% count() %>% pull
tops_factor.tbl %<>% filter(!is.na(issn))
tops_signatory_clean.tbl %>% filter(is.na(issn)) %>% count() %>% pull
tops_signatory.tbl <- issnClean(tops_signatory_clean.tbl)
tops_signatory.tbl %>% filter(is.na(issn)) %>% count() %>% pull
tops_signatory.tbl %<>% filter(!is.na(issn)) %>% mutate(topsSign=TRUE)
...
editors_join.tbl %<>% left_join(tops_factor.tbl %>% select(issn,topScore) ,by=c("issn"))
editors_join.tbl %<>% left_join(tops_signatory.tbl %>% select(issn,topsSign) ,by=c("issn")) %>% rowwise()  %>% mutate(topsSign=ifelse(is.na(topsSign),is.na(issn),topsSign))
```
...
person_ids.tbl <- editors_parse.tbl %>% 
...
editors_parse.tbl %<>% left_join(person_ids.tbl,by=c("editor","affiliation"))
...
if (doc_refresh_data){
  ...
  save(nms.tbl,file=paste("nms_",gender_meth,".RData",sep=""))
} else {
  load( file=paste("nms_",gender_meth,".RData",sep=""))
}
...
editors_analysis.tbl %<>% rowwise() %>%
  mutate(CAT_TOP = case_when(
    ((is.na(topScore)==FALSE) && (topScore > 0)) ~ 3,
    (is.na(topScore)==FALSE) ~ 2,
    ((is.na(topsSign)==FALSE) && topsSign) ~ 1,
    (is.na(topsSign)==FALSE) ~ 0,
#    TRUE ~ NA
  ))
...
editors_analysis.tbl %<>% mutate (
  ...
  select(NM_JOURNAL, CAT_PUBLISHER,CAT_TOP, IND_MALE, IND_OPEN, LS_COUNTRY, LS_SUBJECTS, FAC_ROLE, CAT_TOP,NM_EDID) %>% ungroup()
...
journal_board_analysis.tbl <- editors_analysis.tbl %>% 
  ...
  mutate(LIST_EDID = ifelse(length(unlist(LIST_EDID))==0, NA, LIST_EDID),
         LIST_ROLEGROUP_COUNTRIES = 
           ifelse(length(unlist(LIST_ROLEGROUP_COUNTRIES))==0,NA,LIST_ROLEGROUP_COUNTRIES)
         ) %>%
  ungroup()
...
journal_analysis.tbl <- journal_board_analysis.tbl %>%
  ...
  select(-N_ROLEGROUP_COUNTRIES,-PERCENT_ROLEGROUP_MALE, -LIST_ROLEGROUP_COUNTRIES, -N_JRN_SIZE, -FAC_ROLE) %>% 
  slice_head()  %>%
  ungroup() %>% 
...
journals.grViz <- journals_linked.graph %>% render_graph()
#journals.vz <- journals_linked.graph %>% render_graph(output="visNetwork")
#journals.svg <- journals.graph %>% render_graph(as_svg=TRUE)
...
journals_linked.tgraph %>% ggraph(layout="stress") +
    #geom_edge_fan(aes(color="blue",alpha=.2)) +
    geom_edge_fan(alpha=.2) +
    geom_node_point(aes(color=factor(CAT_PUBLISHER)))
