...
journals_linked.tgraph %>%
  ...
  ggraph(layout="stress") +
    ...
    geom_node_point(mapping=aes(color=Publisher)) +
                      scale_color_viridis_d() + theme(legend.position ="bottom") -> fig5

fig5
fig5 %>% ggsave(plot=., filename="figure_5.png" , units="in", width=7, height=7,scale=2)
...
journal_board_analysis.tbl %>% 
  filter(!is.na(FAC_ROLE)) %>%
  group_by(IND_OPEN,FAC_ROLE) %>%
  summarize(prop_female = 1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(values_from=prop_female, names_from=IND_OPEN) %>%
  gt() %>% 
    cols_label( 
                "FAC_ROLE"="",
                "FALSE"="Closed Access" , 
    ...
    tab_header(title="Board Proportion Female by OA   status") %>%
    data_color (
...
journal_board_analysis.tbl %>% 
  ...
  gt() %>% 
    ...
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Board International Diversity by OA  status") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
...
journal_board_analysis.tbl %>% 
  ...
  mutate(IND_TOP=ifelse(IND_TOP,"TRUE","FALSE")) %>%
  filter(!is.na(FAC_ROLE)) %>%
  group_by(IND_TOP,FAC_ROLE) %>%
  summarize(prop_female = 1-mean(PROPENSITY_ROLEGROUP_MALE,na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(values_from=prop_female, names_from=IND_TOP) %>%
  gt() %>% 
    cols_label( 
                "FAC_ROLE"="",
                "FALSE"="Not Open Science" , 
                "TRUE"="Open Science",
                ) %>%
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Board Proportion Female by TOPS  status") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
...
journal_board_analysis.tbl %>% 
  ...
  summarize( ihhi = 1-mean(PERCENT_HHI,na.rm=TRUE)) %>%
  ungroup() %>%
  pivot_wider(values_from=ihhi, names_from=IND_TOP) %>%
  gt() %>% 
    cols_label( 
                "FAC_ROLE"="",
                "FALSE"="Not Open Science" , 
    ...
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Board International Diversity by TOPS  status") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
...
journal_board_analysis.tbl %>% 
  ...
  gt() %>% 
    ...
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Board Proportion Female by OA and TOPS status") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
    ...
    gt_theme_538()  -> tb3a.gt

tb3a.gt
tb3a.gt %>% gtsave("tb3a.htm",inline_css=FALSE)
...
journal_board_analysis.tbl %>% 
  ...
  gt() %>% 
    ...
    gt_theme_538() ->  tb3b.gt
tb3b.gt %>% gtsave("tb3b.htm",inline_css=FALSE)
