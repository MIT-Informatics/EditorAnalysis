...
editors_analysis.tbl %<>% mutate (
  ...
  rename( CAT_JOURNAL=journal, CAT_PUBLISHER=publisher, LS_SUBJECTS = subjects, IND_OPEN = IND_openlicense) %>%
  select(CAT_JOURNAL, CAT_PUBLISHER, IND_MALE, IND_OPEN, LS_COUNTRY, LS_SUBJECTS, FAC_ROLE) %>% ungroup()

### construct editorial board characteristics
journal_board_analysis.tbl <- editors_analysis.tbl %>% group_by(CAT_JOURNAL,FAC_ROLE) %>% summarise(
  CAT_PUBLISHER = head(CAT_PUBLISHER,n=1),
  LS_SUBJECTS= head(LS_SUBJECTS,n=1),
  IND_OPEN= head(IND_OPEN,n=1),
  LIST_ROLEGROUP_COUNTRIES = paste(na.omit(LS_COUNTRY),collapse=","),
...
journal_analysis.tbl %>% select(-CAT_JOURNAL) %>% Desc()
```
...
```{r analysis-edboards}
journal_board_analysis.tbl %>% select(-CAT_JOURNAL,-LIST_ROLEGROUP_COUNTRIES) %>% Desc(~.,data=.)
journal_board_analysis.tbl %>%
  Desc(IND_OPEN~N_ROLEGROUP_COUNTRIES+PERCENT_ROLEGROUP_MALE,data=.)
...
lm.res <- journal_board_analysis.tbl %>% 
  lm(PERCENT_ROLEGROUP_MALE~IND_OPEN+FAC_ROLE,data=.)
...
#TODO: Add nominal types
#TODO: iterate through explore.ls
...
iris %>% mixed_assoc(df=.)
 # mixed_assoc(journal_board_analysis.tbl)
 

assoc.tbl <- journal_board_analysis.tbl %>%
  select(-CAT_JOURNAL) %>%
    mixed_assoc(df=.) 
