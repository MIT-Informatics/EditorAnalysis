...
gender_meth <- "ssa"
editors_full.tbl %<>% left_join(gender_select.bl <- nmsgender.tbl %>% filter(method==gender_meth) %>% select(name,gender,proportion_male) %>% rename(LS_GIVENNAME=name,LS_GENDER=gender,PERCENT_PROPENSITY_MALE=proportion_male)
, by=c("LS_GIVENNAME"))
...
journal_analysis.tbl %>% gf_bar(~CAT_PUBLISHER, sort=stat_count()) + coord_flip()
```
...
#TODO: Fix crosstable kludge -- cleaner with group_by?
 ...
 edcountry.gt <- edcountry.ct %>% arrange(desc(as.numeric(Total))) %>% mutate(Percentage=0+as.numeric(Total)/sum(as.numeric(Total))) %>% slice_head(n=20) %>% as_gt 
...
editors_analysis.tbl %>% 
  ...
  gt() %>% 
    ...
    fmt_percent(columns=matches("FALSE|TRUE"),decimals=0) %>%
    tab_header(title="Propensity Male by OA and TOPS status") %>%
    data_color (
      columns = matches("FALSE|TRUE"), 
      colors = scales::col_numeric( 
      palette = c( "white","orange"), 
...
p1<- tmp %>% gf_dens2(~PERCENT_HHI|FAC_ROLE,fill=~IND_OPEN)
p2<- tmp %>% gf_dens2(~PROPENSITY_ROLEGROUP_MALE|FAC_ROLE,fill=~IND_OPEN)
p3<- tmp %>% gf_dens2(~PERCENT_HHI|FAC_ROLE,fill=~IND_TOP)
p4<- tmp %>% gf_dens2(~PROPENSITY_ROLEGROUP_MALE|FAC_ROLE,fill=~IND_TOP)
plot((p1+p2)/(p3+p4))
```
...
#TODO: Change universe to all journals
clustjournals.tib %>% unnest(LIST_EDID) %>% count(LIST_EDID,sort=TRUE) %>%
   count(n) %>% 
   gt %>% 
    cols_label(n="Number of journals",nn="") %>%
    tab_header(title="Number of journals served by editors in large clusters") %>%
    gt_theme_538()
